        ...
        phases: {
          install: {
            ...
            commands: [
              ...
              'echo "export PATH=$(pwd)/apache-maven-3.2.5/bin:$PATH" >> ~/.bashrc',
              'mvn --version',
            ],
          },
          pre_build: {
            commands: [
              ...
              'aws secretsmanager get-secret-value --secret-id dockerhub-credentials --query SecretString --output text | jq -r \'.password\' | docker login -u $DOCKERHUB_USERNAME --password-stdin',
              'echo Logging in to Amazon ECR...',
              'aws ecr get-login-password --region $AWS_REGION | docker login --username AWS --password-stdin $AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com',
              'cd backend',
              'echo Skipping tests for now...',
              'echo "Maven version: $(mvn --version)"',
              'export MAVEN_OPTS="-Dmaven.test.skip=true -Dmaven.repo.local=$PWD/.m2/repository"',
        ...
        phases: {
          ...
          post_build: {
            commands: [
              ...
              'ls -la',
              'cd frontend/advisor-portal || true',
              'if [ -d "build" ]; then aws s3 sync build/ s3://avalon-frontend-$AWS_ACCOUNT_ID-$AWS_REGION/ --delete; else cd ../../ && aws s3 sync frontend/advisor-portal/build/ s3://avalon-frontend-$AWS_ACCOUNT_ID-$AWS_REGION/ --delete; fi',
